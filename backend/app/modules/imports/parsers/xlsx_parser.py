"""XLSX parser for Clear corretora PosicaoDetalhada.xlsx position snapshots.

Clear's PosicaoDetalhada.xlsx is a position snapshot (NOT transaction history).
We convert each position to a synthetic 'buy' transaction using:
  - preço médio (average cost) as unit_price
  - quantidade as quantity
  - transaction_date = file extraction date (from cell A1) or today

Sheet layout ("Sua carteira"):
  Row 1:  "Conta: XXXXXXXX | DD/MM/YYYY, HH:MM" — account info + date
  Row 8:  Ações section header row  → [Ativo, Posição(R$), %Aloc, Rent, Preço Médio, Último Preço, Qtd Total]
  Row 9+: Ações data rows until blank
  Row 29: FIIs section header row → [Ativo, Posição(R$), %Aloc, Rent c/Prov, Rent Bruta, Preço Médio(Abertura), Última Cotação, Qtd Cotas]
  Row 30+: FIIs data rows until blank

Column indices (0-based after openpyxl min_column offset):
  Ações:   col A=ticker, E=preço_médio, G=qtd_total
  FIIs:    col A=ticker, F=preço_médio_abertura, H=quantidade_de_cotas
"""
...
def parse_xlsx(xlsx_bytes: bytes) -> tuple[list[dict], list[str]]:
    ...
    if is_xp:
        logger.info("xlsx_parser: detected XP format")
...
def parse_clear_xlsx(xlsx_bytes: bytes) -> tuple[list[dict], list[str]]:
    ...
    Returns:
        Tuple of (transactions, warnings) where:
        - transactions: list of dicts compatible with _write_staging_rows
    ...
    # Find ações section (look near row 8 for "Ativo" or "ação" label)
    acoes_header = _find_section_header_row(ws, _ACOES_HEADER_ROW, "ativo", tolerance=5)
    acoes = _parse_acoes_section(ws, acoes_header, txn_date)
    if acoes:
        transactions.extend(acoes)
        logger.info("xlsx_parser: parsed %d ações", len(acoes))
    else:
        warnings.append("Nenhuma ação encontrada na seção Ações")

    # Find FIIs section (look near row 29)
    fiis_header = _find_section_header_row(ws, _FIIS_HEADER_ROW, "ativo", tolerance=8)
    fiis = _parse_fiis_section(ws, fiis_header, txn_date)
    if fiis:
        transactions.extend(fiis)
        logger.info("xlsx_parser: parsed %d FIIs", len(fiis))
    else:
