...
function DeltaRow({ item }: { item: WizardDeltaItem }) {
  const actionClass = ACTION_COLORS[item.action] ?? "text-zinc-500";
  return (
      ...
      {Array.from({ length: total }, (_, i) => i + 1).map((n) => (
        <div key={n} className="flex items-center">
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs text-zinc-500 mt-1">{STEP_LABELS[n - 1]}</span>
          </div>
          {n < total && (
            <div className={`h-0.5 w-8 mx-1 mb-5 ${n < current ? "bg-blue-500" : "bg-zinc-200"}`} />
          )}
        </div>
      ))}
    </div>
...
export function WizardContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      {(isIdle || isFailed) && (
        <>
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-5">
            ...
            {step === 1 && (
              <div>
                <h2 className="text-sm font-bold text-zinc-800 mb-3">Quanto voce quer investir?</h2>
                ...
                <input
                  ...
                  className="w-44 rounded-lg border border-zinc-200 px-3 py-2 text-sm focus:outline-none focus:border-blue-400"
                />
                <p className="text-xs text-zinc-400 mt-1">Minimo: R$ 100</p>
            ...
            {step === 2 && (
              <div>
                <h2 className="text-sm font-bold text-zinc-800 mb-3">Qual o prazo do investimento?</h2>
                <label className="block text-xs font-semibold text-zinc-600 mb-1.5">Horizonte de investimento</label>
            ...
            {step === 3 && (
              <div>
                <h2 className="text-sm font-bold text-zinc-800 mb-3">Qual o seu perfil de risco?</h2>
                <label className="block text-xs font-semibold text-zinc-600 mb-1.5">Perfil de risco</label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {PERFIS.map((p) => {
                    const isSelected = perfil === p.key;
                    const borderMap: Record<string, string> = {
                      emerald: isSelected ? "border-emerald-500 bg-emerald-50" : "border-zinc-200 hover:border-emerald-300",
                      blue:    isSelected ? "border-blue-500 bg-blue-50"       : "border-zinc-200 hover:border-blue-300",
                      purple:  isSelected ? "border-purple-500 bg-purple-50"   : "border-zinc-200 hover:border-purple-300",
                    };
                    const textMap: Record<string, string> = {
                      emerald: isSelected ? "text-emerald-700" : "text-zinc-700",
                      blue:    isSelected ? "text-blue-700"    : "text-zinc-700",
                      purple:  isSelected ? "text-purple-700"  : "text-zinc-700",
                    };
                    return (
                      <button
                        key={p.key}
      ...
      {/* Loading */}
      {isProcessing && (
        <div className="rounded-xl border border-zinc-200 bg-white p-8 flex flex-col items-center gap-4 text-center">
          <div className="h-10 w-10 rounded-full border-4 border-blue-500 border-t-transparent animate-spin" />
          <div>
            <p className="text-sm font-semibold text-zinc-700">Analisando seu perfil…</p>
      ...
      {isDone && job?.result && (
        <>
          ...
          {job.result.macro && Object.keys(job.result.macro).length > 0 && (
            <div className="flex flex-wrap gap-2">
              {Object.entries(job.result.macro).map(([k, v]) => (
                ...
                >
                  <span className="uppercase font-semibold text-zinc-500">{k}</span>
                  <span>{v}</span>
                </span>
              ))}
            </div>
          ...
          {job.result.allocation.rationale && (
            <div className="rounded-xl border border-zinc-200 bg-white p-5">
              <h2 className="text-sm font-bold text-zinc-800 mb-2">Justificativa</h2>
              <p className="text-sm text-zinc-600 leading-relaxed">{job.result.allocation.rationale}</p>
            </div>
          )}
          ...
          {job.result.delta && job.result.delta.length > 0 && (
            <div className="rounded-xl border border-zinc-200 bg-white p-5">
              <h2 className="text-sm font-bold text-zinc-800 mb-1">
