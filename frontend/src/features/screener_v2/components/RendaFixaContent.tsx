...
function typeBadge(type: string) {
  ...
  const cls = colors[type] ?? "bg-zinc-100 text-zinc-600";
    ...
    <tr className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
      <td className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-500">{tenorStr}</span>
        </div>
            ...
            {(cdiForPeriod !== null || ipcaForPeriod !== null) && (
              <div className="text-xs mt-0.5">
                {beatsCDI ? (
                  ...
                  <span className="text-zinc-400">— abaixo</span>
                )}
              </div>
            )}
          </td>
...
function TesouroRow({ row }: { row: TesouroRateRow }) {
  return (
    <tr className="border-b border-zinc-100 hover:bg-zinc-50 transition-colors">
      ...
      <td className="py-3 px-4">
        <span className="text-xs px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-500">{row.source}</span>
      </td>
    </tr>
  );
}
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {typeFilter !== "Tesouro" && (
          <div className="flex items-center gap-2">
            <label className="text-xs text-zinc-500">Prazo min (meses):</label>
            <input
              type="number"
              value={minMonths}
              onChange={(e) => setMinMonths(e.target.value)}
              placeholder="ex: 12"
              className="w-20 px-2 py-1.5 rounded-md border border-zinc-200 text-xs"
              min="0"
            />
          </div>
        )}
        ...
        {typeFilter !== "Tesouro" && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-zinc-500">Ordenar por retorno:</span>
        ...
        {typeFilter !== "Tesouro" && (
          <span className="text-xs text-zinc-400">{filteredCatalog.length} produto(s)</span>
        )}
      </div>
      ...
      {(typeFilter === "" || typeFilter === "Tesouro") && (
        <section>
          <div className="mb-3">
            ...
            <p className="text-xs text-zinc-500">Taxas indicativas atualizadas a cada 6h via ANBIMA</p>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Título</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Vencimento</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Taxa</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">PU</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Data base</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Fonte</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loadingTesouro
            ...
            <div className="rounded-lg border border-zinc-200 bg-white overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-zinc-50 border-b border-zinc-200">
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Produto / Prazo</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">Taxa bruta</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">6 meses</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">1 ano</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">2 anos</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-zinc-600">5 anos</th>
                    </tr>
                  </thead>
                  <tbody>
                    {loadingCatalog
