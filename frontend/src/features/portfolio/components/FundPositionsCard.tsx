...
export function FundPositionsCard() {
  ...
  if (isLoading) return <ShimmerSkeleton className="h-32 rounded-xl" />;
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Buildings className="h-4 w-4 text-indigo-500" weight="fill" />
          <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">Fundos de Investimento</p>
        </div>
        <div className="flex items-center gap-3 text-xs text-zinc-400">
          <span>Total investido: <span className="font-semibold text-zinc-800">{fmtBrl(totalCost)}</span></span>
          {totalPnl !== 0 && (
          ...
          <tbody className="divide-y divide-zinc-50">
            {positions.map((p, i) => {
              ...
              return (
                ...
                >
                  <td className="py-2">
                    <p className="font-medium text-zinc-800 text-xs leading-tight max-w-[200px] truncate" title={p.name}>
                      {p.name}
                    </p>
